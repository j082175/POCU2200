...
int get_character(const char* filename, character_v3_t* out_character)
{
...

    {
        ...
        case 3:
            ...
            /* 1열  토큰화 */
            strcpy(backup_buffer2, buffer);

            limit = count_token(backup_buffer2, '|') + 1;
            {
                int i;
                for (i = 0; i < limit; i++) {
                    temp = strtok(ptr, "|  \n");
            ...
            strcpy(backup_buffer2, buffer + total_count);
            temp = NULL;
            {
                int i;
                int limit = count_token(backup_buffer2, '|') + 1;
                ptr = backup_buffer2;
            ...
            if (out_character->minion_count) {
                ...
                strcpy(backup_buffer2, buffer + total_count + 1);
                total_count += string_length_before_carriage_return(buffer + total_count + 1) + 1;
                /*char* b = backup_buffer2 + total_count + 1;*/
                if (*(backup_buffer2 + total_count + 1) != '\0') {
                    {
                        int i;
                        int limit = count_token(backup_buffer2, '|') + 1;
                        ptr = backup_buffer2;

                        /*total_count += string_length_before_carriage_return(backup_buffer2 + total_count + 1) + 1;*/
